...
export default class Cast extends Expression{
    ...
    public execute(env: any):Return {
        ...
        switch (this.type) {
            case Type_dxnry.INT:
                return { value: parseInt(value.value), type: Type_dxnry.BOOLEAN }
            case Type_dxnry.DOUBLE:
                return { value: parseFloat(value.value), type: Type_dxnry.BOOLEAN }
            case Type_dxnry.STRING:

                return { value: value.value.toString(), type: Type_dxnry.BOOLEAN }
